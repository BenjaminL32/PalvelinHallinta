...
First steps to take in this assignment is to start up virtual machines.  
...
After that, I will create a .sls file inside the newly created /hello folder with the command `sudoedit init.sls`  
...
For now I will only write local commands, running there commands to another machine comes in a future step. 
<img width="211" alt="image" src="https://github.com/user-attachments/assets/b6535880-2eb5-4ed4-887f-43a457777a9d" />  
(Hello Salt Infra-as-code, terokarvinen.com)  
</br>
Now that the file has been created and commands written, I´ll try to run the command `sudo salt-call --local state.apply hello`.  
...
First steps to making the the .sls file, is opening the file previously made, and adding more functions to it. 
